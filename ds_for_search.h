...
public:
    ...
    ~PriorityQueue() {
        for (int i = 0; i <= this->rear; ++i) {
            delete this->queue[i];  // Delete each dynamically allocated Node
    ...
    void UpdateNode(const T& item) {
        // First, find the node's position in the queue
        int nodeIndex = -1;
        for (int i = 0; i <= this->rear; ++i) {
        ...
        if (nodeIndex == -1) {
            // The node was not found, so we can't update it
            return;
        }

        // Remove the node from its current position
        for (int i = nodeIndex; i < this->rear; ++i) {
            this->queue[i] = this->queue[i + 1];
        }
        this->rear--;
        this->count--;

        // Re-insert the node with its updated priority
