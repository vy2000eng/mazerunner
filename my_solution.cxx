...
void PlayerRandomMove::Init(const MazeElem player, const int maze_rows, const int maze_cols)
{
    ...
    totalCells = i_m_rows * i_m_cols;
    explored_cells_count = 0;
    baseThreshold= (maze_rows * maze_cols) * .5;
...
void PlayerRandomMove::update_maze(const Loc loc, const IMaze *pMaze) {
    ...
    for (int r = max(0, loc.r - fogBoundary); r <= min(i_m_rows - 1, loc.r + fogBoundary); ++r) {
        for (int c = max(0, loc.c - fogBoundary); c <= min(i_m_cols - 1, loc.c + fogBoundary); ++c) {
            ...
            if(internal_maze[r][c] == '?')
                explored_cells_count++;
...
//        return ReverseMove(moves_taken.Pop());
    if (!path.IsEmpty() && path_valid) {
        ...
        if (is_passable_internal_matrix(nextLoc)) {
    ...
    if (!path_valid || path.IsEmpty()) {
        target = DetermineNextTarget(loc, maze, iter);
    ...
    while (!moves_taken.IsEmpty()) {
        ...
        if ( is_passable_internal_matrix(newLoc)) {
            return reverseMove;
        }
    }
...
Loc PlayerRandomMove::DetermineNextTarget(const Loc &currentLoc, const IMaze *pMaze, const int i) {
   ...
   // double bestScore = std::numeric_limits<double>::max();
    ...
    //int visibilityRange = 4;
    double explorationProgress = get_exploration_progress()*100;
    double threshold = calculate_adjusted_threshold();
    Loc referencePoint = explorationProgress< threshold ? currentLoc : pMaze->GetExitLoc();
    for (int r = max(0, currentLoc.r - visibilityRange); r <= min(i_m_rows - 1, currentLoc.r + visibilityRange); ++r) {
        for (int c = max(0, currentLoc.c - visibilityRange); c <= min(i_m_cols - 1, currentLoc.c + visibilityRange); ++c) {
            ...
            if (!is_passable_internal_matrix(referencePoint)
            )
                continue;
            //if(back_tracking && targets[potentialTarget.r][potentialTarget.c]) continue;
            double score = CalculateTargetScore(  potentialTarget,referencePoint);
    ...
    if (!foundTarget) {
        do {
            ...
            randomRow = max(0, min(randomRow, i_m_rows-1));
            randomCol = max(0, min(randomCol, i_m_cols)-1);
            target = Loc(randomRow, randomCol);
...
bool PlayerRandomMove::AStarSearch(const IMaze *pMaze, Loc begin, Loc target) {
    ...
    while (!openSet.IsEmpty()) {
        ...
        for (Direct dir: {UP, DOWN, LEFT, RIGHT}) {
            ...
            if (!is_passable_internal_matrix(neighborLoc) || visited_loc[neighborLoc.r][neighborLoc.c]|| closed_set[neighborLoc.r][neighborLoc.c])
                continue;
            //if (closed_set[neighborLoc.r][neighborLoc.c]) continue;

            double tentativeGCost = current->gCost + 1;
            Node *neighborNode = new Node(neighborLoc, current, tentativeGCost,
                                          Node::CalculateHCost(neighborLoc, pMaze->GetExitLoc()));
...
double PlayerRandomMove::get_exploration_progress() {
   // int totalCells = i_m_rows * i_m_cols;

    double r =  static_cast<double>(explored_cells_count) / totalCells;
    return r;

}

const double EXPLORATION_PROGRESS_THRESHOLD = 0.3;
const double THRESHOLD_ADJUSTMENT_COEFFICIENT = 0.70;
const double PROGRESS_SCALE = 100.0;

double PlayerRandomMove::calculate_adjusted_threshold() {
    double progress = get_exploration_progress();

    if (progress > EXPLORATION_PROGRESS_THRESHOLD && baseThreshold > progress * PROGRESS_SCALE) {
        baseThreshold *= THRESHOLD_ADJUSTMENT_COEFFICIENT;
    }
